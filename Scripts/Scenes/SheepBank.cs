...
public class SheepBank : Mz_BaseScene {
	...
	public GameObject limitVersionWindow_obj;
...
//		sheepBankTutor = new SheepBankTutor();
    ...
    void CreateBuyUpgradeShopTutorEvent()
    {
		...
		sheepBankTutor.upgrade_blueberryJam_button.GetComponent<tk2dSprite>().spriteId = 25;
    ...
    public override void OnInput (string nameInput)	
	{
        ...
        else if (nameInput == BACK_BUTTON_NAME)
        {
            if (upgradeInside_window_Obj.active)
            {
				...
				if(limitVersionWindow_obj.activeSelf) {
					limitVersionWindow_obj.SetActive(false);
					shadowPlane_Obj.transform.position = new Vector3(0,0,-10);
				}
        ...
        switch (currentGameStatus)
        {
            case GameSceneStatus.ShowUpgradeInside:
				...
				else if(nameInput == limitVersionWindow_obj.name) {
